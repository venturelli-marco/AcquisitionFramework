...
class AcquisitionKinect(acquisition.Acquisition):
    def __init__(self, resolution_mode=acquisition.RESOLUTION_FULLHD):
    ...
    def get_depth_frame(self):
        ...
        self._frameDepth = self._frameDepth.reshape(acquisition.RESOLUTION_DEPTH).astype(np.uint16)
    ...
    def acquireFrame(self):
        # hand = [0.6642667055130005, 0.39338287711143494, 1.3229750394821167]
        # hand_j = PyKinectV2._Joint()
        # hand_j.Position.x = hand[0]
        # hand_j.Position.y = hand[1]
        # hand_j.Position.z = hand[2]
        # xy = self._kinect._mapper.MapCameraPointToDepthSpace(hand_j.Position)
        # xy_rgb = self._kinect._mapper.MapCameraPointToColorSpace(hand_j.Position)

        if self._kinect.has_new_depth_frame():
            self.get_depth_frame()
            # self.draw_depth_frame()
            # self._frameSkeleton = self._frameDepth.copy()
            # self._frameSkeleton = np.dstack((self._frameSkeleton,self._frameSkeleton,self._frameSkeleton))
            # self._frameDepth = None

        if self._kinect.has_new_color_frame():
            self.get_color_frame()
            # self.draw_color_frame()
            # self._frameRGB = None
        ...
        #     ir_image = self._kinect.get_last_infrared_frame()
        #     ir_image = ir_image.reshape(acquisition.RESOLUTION_DEPTH)
        #     cv2.normalize(ir_image, ir_image, 0,255, cv2.NORM_MINMAX)
        #     # ir_image = ir_image.astype(np.int32) / 256
        #     cv2.imshow("infrared", ir_image.astype(np.uint8))
